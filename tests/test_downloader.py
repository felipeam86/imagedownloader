...
def test_download():
    ...
    downloaded = len([path for path in paths if path is not None])

    assert Path(
        store_path.name
    ).exists(), f"Image directory {store_path} should exist after download"
...
def test_wrong_url_on_iterable_returns_none():
    ...
    assert paths[0] is None, (
        "If image from an iterable cannot be downloaded, " "it should return None"
    )
